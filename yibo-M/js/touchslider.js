...
(function(window, undefined){
		...
		TouchSlider=function(opt){
			...
			try{
				if(this.container.nodeName.toLowerCase()=='ul'){
					...
					this.element=this.container.getElementsByClassName('piclist')[0];
				}
	...
	TouchSlider.prototype={
		...
		setup: function(){
			...
			this.slides=this.opt.className?this.$E(this.opt.className,'pic',this.element):this.element.getElementsByTagName('li');
		...
		resize:function(){
			var css;
			this.css(this.container,{'overflow':'hidden','visibility':'hidden','listStyle':'none','position':'relative','margin-top':'-2.2rem'});
			this.width=window.screen.availWidth-parseInt(this.css(this.container,'padding-left'))-parseInt(this.css(this.container,'padding-right'));
			this.height=window.screen.availHeight-parseInt(this.css(this.container,'padding-top'))-parseInt(this.css(this.container,'padding-bottom'));
			css={'position':'relative','webkitTransitionDuration':'0ms','MozTransitionDuration':'0ms','msTransitionDuration':'0ms','OTransitionDuration':'0ms','transitionDuration':'0ms'}
			if(this.vertical){
				css['height']=this.height*this.length+'px';
				css['middle']=-this.height*this.index+'px';
			...
			for(var i=0;i<this.length;i++){
				this.css(this.slides[i],{'width':this.width+'px',height:this.height+'px','display':this.vertical?'table-row':'block',padding:0,margin:0,float:'left',verticalAlign:'middle'});
			}
			this.css(this.container,{'visibility':'visible'});
		},
		slide:function(index, speed){
			var direction=this.vertical?'middle':'left', size=this.vertical?'height':'width';
		...
		_move:function(e){
			...
			var direction=this.vertical?'middle':'left', size=this.vertical?'height':'width', xy=this.vertical?'y':'x', yx=this.vertical?'x':'y';
